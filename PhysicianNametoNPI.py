...
import openpyxl
class PhysicianName:
  def __init__(self, first_name, last_name, state, row, second_last_name):
...
def get_physician_name_from_excel(file):
    ...
    sh = wrkbk['Triad HealthCare Network']

    # iterate through excel and display data
    for i in range(1200, sh.max_row+1):
        state = sh.cell(row = i, column = 4).value
        second_last_name = sh.cell(row = i, column = 3).value
        physician_name = PhysicianName(sh.cell(row=i, column = 1).value, sh.cell(row=i, column=2).value, state, i, second_last_name)
...
def get_npi_from_name(physician_name, file, wrkbk, sh):
    ...
    wrkbk.save("/Users/ptran2/Downloads/ACOs.xlsx")

get_physician_name_from_excel("/Users/ptran2/Downloads/ACOs.xlsx")
